...
from .model import CLIP, CustomTextCLIP, convert_weights_to_lp, convert_to_custom_text_state_dict,\
    resize_pos_embed, get_cast_dtype, resize_text_pos_embed, set_model_preprocess_cfg, TimeSeriesCfg
from .coca_model import CoCa
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, TimeSeriesClipLoss
...
_MODEL_CONFIGS['aaabatterytest'] = {
    ...
    "text_cfg": {
        ...
        "width": 512,  #768
        "context_length": 77, # <-- 新增：明确指定上下文长度
        "output_tokens":False,
        # "lock_text": True,
    },
...
_MODEL_CONFIGS['btr-1024-qwen2.5'] = {
    "embed_dim": 1024,
    ...
    "text_cfg": {
        "hf_model_name": "/mnt/disk1/fzm/codes/Time-LLM/pretrained_model/Qwen2.5-7B",
        "hf_tokenizer_name": "/mnt/disk1/fzm/codes/Time-LLM/pretrained_model/Qwen2.5-7B",
        "hf_proj_type": "linear",
        "hf_model_pretrained": True,
        "width": 1024,  #768
        "context_length": 90, # <-- 新增：明确指定上下文长度
        "output_tokens":False,
        # "lock_text": True,
    },
    "custom_text": False,
...
_MODEL_CONFIGS['btr-2048-qwen2.5'] = {
    "embed_dim": 2048,
    ...
    "text_cfg": {
        ...
        "hf_model_pretrained": True,
        "width": 2048,  #768
        "context_length": 100, # <-- 新增：明确指定上下文长度
        "output_tokens":False,
        # "lock_text": True,
    },
    "custom_text": False,
...
_MODEL_CONFIGS['btr-2048-vicuna7b'] = {
    "embed_dim": 2048,
    ...
    "text_cfg": {
        "hf_model_name": "/mnt/disk1/fzm/codes/Time-LLM/pretrained_model/Vicuna-7b",
        "hf_tokenizer_name": "/mnt/disk1/fzm/codes/Time-LLM/pretrained_model/Vicuna-7b",
        "hf_proj_type": "linear",
        "hf_model_pretrained": True,
        "width": 2048,  #768
        "context_length": 100, # <-- 新增：明确指定上下文长度
...
_MODEL_CONFIGS['btr-512-bert-sl40'] = {
    "embed_dim": 512,
...
_MODEL_CONFIGS['btr-1024-bert-sl40'] = {
    "embed_dim": 1024,
    ...
    "text_cfg": {
        ...
        "hf_model_pretrained": True,
        "width": 1024,  #768
        "context_length": 70, # <-- 新增：明确指定上下文长度
        "output_tokens":False,
    },
    # "multimodal_cfg": {
    #     "context_length": 76,
...
def _rescan_model_configs():
    ...
    for cf in config_files:
        with open(cf, 'r') as f:
            ...
            if all(a in model_cfg for a in ('embed_dim', 'vision_cfg', 'text_cfg', 'timeseries_cfg')):
...
def create_model(
        ...
        force_context_length: Optional[int] = None,
                # --- [修改 4] ---
        # 增加与时序模型相关的强制覆盖参数
        force_timeseries_seq_len: Optional[int] = None,
    ...
    if 'timeseries_cfg' in model_cfg:
        timeseries_cfg = model_cfg['timeseries_cfg']
        if force_timeseries_seq_len is not None:
            timeseries_cfg["seq_len"] = force_timeseries_seq_len
    else:
        # 这个错误理论上不会发生，因为 _rescan_model_configs 已经检查过了
        raise ValueError("Model config must contain either 'vision_cfg' or 'timeseries_cfg'")
    ...
    # --- [修改 6] ---
    # 这里的 model_class 就是我们修改后的 CLIP (即 MultiModalModel)
    # 它的 __init__ 函数已经可以接受 vision_cfg 或 timeseries_cfg
    ...
    partially_loaded = enable_default_text_weights or pretrained_text_loaded
    ...
    elif model.encoder_type == 'timeseries':
        # 对于时序模型，我们可以附加一个空的预处理配置，或者附加时序相关的配置
        # 这里我们暂时不处理，因为时序的预处理将在 data.py 中完成
        logging.info("TimeSeries model created. Preprocessing should be handled in the data pipeline.")
...
def get_tokenizer(
    ...
    print(f"Text Context length set to {context_length} from config or default.")
...
def _set_model_device_and_precision(
    ...
    if precision in ("fp16", "bf16"):
        ...
        # --- [修改] ---
        # 即使不是 timm 模型，我们也可能使用了 LayerNormFp32 (例如在 transformer.py 中)
        # 所以我们总是检查并转换 LayerNormFp32
        ...
        # 如果需要使用 convert_weights_to_lp (OpenAI 原始逻辑)，可以在这里保留
        # 但通常 model.to(dtype) + LayerNormFp32 fix 已经足够
        # if not is_timm_model:
...
def create_loss(args):
    ...
    # elif getattr(args, 'model_type', 'vision') == 'timeseries':
    #     logging.info("Creating TimeSeriesClipLoss.")
    #     return TimeSeriesClipLoss(
    #         local_loss=args.local_loss,
    #         gather_with_grad=args.gather_with_grad,
    #         cache_labels=True,
    #         rank=args.rank,
...
def create_model_and_transforms(
    ...
    model = create_model(
        ...
        weights_only=weights_only,
        force_timeseries_seq_len=force_timeseries_seq_len,
        **model_kwargs,
    )
    ...
    if model.encoder_type == 'vision':
        ...
        # 对于时序模型，训练和验证的预处理可能只是简单的归一化或填充
        # 这里我们返回 None，并假设这些操作在 Dataset 中完成
        # 您也可以在这里定义一个简单的 lambda 函数，例如：
        # preprocess = lambda ts: (ts - mean) / std
        preprocess_train = None
        preprocess_val = None
        logging.info("TimeSeries model selected. Returning None for transforms. Implement preprocessing in the dataset.")
        return model, preprocess_train, preprocess_val
    else:
        raise NotImplementedError(f"Model encoder type '{model.encoder_type}' not supported in create_model_and_transforms.")
