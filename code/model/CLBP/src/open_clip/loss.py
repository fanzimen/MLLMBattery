...
class TimeSeriesClipLoss(ClipLoss):
    """
    这是一个专门为时序数据设计的 ClipLoss 版本。
    它继承了 ClipLoss 的所有核心逻辑（如分布式特征收集、标签生成等），
    但它的 forward 方法明确接受 'timeseries_features' 作为参数，
    使得代码意图更加清晰。
    """
    def forward(
            self,
        ...
        # 直接调用父类的 forward 方法，只需将 timeseries_features 作为 image_features 传入即可。
        # 因为父类的内部逻辑是模态无关的，只关心两个特征矩阵的计算。
        return super().forward(
            image_features=timeseries_features,
