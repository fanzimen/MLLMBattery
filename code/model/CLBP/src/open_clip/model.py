...
# --- [修改 1] ---
# 引入新增的 TimeSeriesTransformer 和 Transformer (因为它在 TimeSeriesTransformer 中被使用)
from .timeseries_model import TimeSeriesTransformer
from .transformer import (
    ...
    text_global_pool,
    # lock_text_tower,
    # Transformer, # 确保 Transformer 被引入
)
from .utils import to_2tuple


# --- [修改 2] ---
# 新增 TimeSeries 配置类，与 CLIPVisionCfg 类似
@dataclass
class TimeSeriesCfg:
    # --- 您需要根据您的数据和模型调整这些参数 ---
    input_dim: int = 12  # 例如：12 维时序数据
    seq_len: int = 1024  # 例如：序列长度为 1024
    patch_size: int = 16   # 每个 patch 的时间步数
    # -----------------------------------------
    ...
    heads: int = 8
    mlp_ratio: float = 4.0
    dropout: float = 0.1  # 注意力和 MLP 中的 dropout 概率
...
# --- [修改 3] ---
# 新增 _build_timeseries_tower 函数
def _build_timeseries_tower(
        embed_dim: int,
        timeseries_cfg: TimeSeriesCfg,
        cast_dtype: Optional[torch.dtype] = None
):
    """
    构建时序编码塔
    """
    if isinstance(timeseries_cfg, dict):
        timeseries_cfg = TimeSeriesCfg(**timeseries_cfg)

    act_layer = nn.GELU
    norm_layer = LayerNormFp32 if cast_dtype in (torch.float16, torch.bfloat16) else LayerNorm
    if timeseries_cfg.norm_kwargs:
        norm_layer = partial(norm_layer, **timeseries_cfg.norm_kwargs)
    if timeseries_cfg.act_kwargs is not None:
        act_layer = partial(act_layer, **timeseries_cfg.act_kwargs)

    # 实例化我们新增的 TimeSeriesTransformer
    timeseries_tower = TimeSeriesTransformer(
        input_dim=timeseries_cfg.input_dim,
        patch_size=timeseries_cfg.patch_size,
        width=timeseries_cfg.width,
        layers=timeseries_cfg.layers,
        heads=timeseries_cfg.heads,
        mlp_ratio=timeseries_cfg.mlp_ratio,
        output_dim=embed_dim,
        act_layer=act_layer,
        norm_layer=norm_layer,
        seq_len=timeseries_cfg.seq_len,
    )

    return timeseries_tower
...
# --- [修改 4] ---
# 将 CLIP 类重命名为 MultiModalModel，并修改构造函数和 forward 方法
class MultiModalModel(nn.Module):
    ...
    def __init__(
            self,
            embed_dim: int,
            # 使 vision_cfg 和 timeseries_cfg 成为可选
            vision_cfg: Optional[CLIPVisionCfg] = None,
            timeseries_cfg: Optional[TimeSeriesCfg] = None,
            text_cfg: Optional[CLIPTextCfg] = None,
            quick_gelu: bool = False,
        ...
        #     self.encoder = _build_vision_tower(embed_dim, vision_cfg, quick_gelu, cast_dtype)
        #     self.encoder_type = 'vision'
        if timeseries_cfg is not None:
            self.encoder = _build_timeseries_tower(embed_dim, timeseries_cfg, cast_dtype)
            self.encoder_type = 'timeseries'
        else:
            raise ValueError("Either vision_cfg or timeseries_cfg must be provided.")
    ...
    def lock_encoder(self, unlocked_groups=0, freeze_bn_stats=False):
        """ 锁定非文本编码器 """
        if hasattr(self.encoder, 'lock'):
            # 兼容 Vision 和 TimeSeries 的 lock 方法
            self.encoder.lock(unlocked_groups=unlocked_groups, freeze_bn_stats=freeze_bn_stats)
    ...
    def lock_text_tower(self, unlocked_layers: int = 0, freeze_layer_norm: bool = True):
        # --- [核心修改] ---
        # 调用 text tower 自己的 lock 方法
        if hasattr(self.text, 'lock'):
            self.text.lock(unlocked_layers, freeze_layer_norm)
        else:
            # 如果 text tower 没有 lock 方法，则手动冻结
            logging.warning("Text tower does not have a .lock() method. Freezing all parameters.")
    ...
    def set_grad_checkpointing(self, enable=True):
        ...
        # --- [修正 3] ---
        # grad_checkpointing 应该在 text tower 上设置
    ...
    def no_weight_decay(self):
        ...
        if hasattr(self.encoder, 'no_weight_decay'):
            # 使用 'encoder.' 前缀
            for n in self.encoder.no_weight_decay():
                no_wd.add('encoder.' + n)
        return no_wd

    # --- [修改 4.2] ---
    # 新增 encode_timeseries 方法
    def encode_timeseries(self, timeseries, normalize: bool = False):
        if self.encoder_type != 'timeseries':
            raise TypeError("The model encoder is not a TimeSeriesTransformer.")
        features = self.encoder(timeseries)
    ...
    def forward(
        ...
        if image is not None:
            encoder_features = self.encode_image(image, normalize=True)
        elif timeseries is not None:
            encoder_features = self.encode_timeseries(timeseries, normalize=True)
        else:
        ...
        if self.output_dict:
            out_dict = {
                # 根据编码器类型，返回对应的特征
                f"{self.encoder_type}_features": encoder_features,
        ...
        if self.logit_bias is not None:
            return encoder_features, text_features, self.logit_scale.exp(), self.logit_bias
        return encoder_features, text_features, self.logit_scale.exp()

# ... (CustomTextCLIP, convert_weights_to_lp 等函数的修改比较复杂且可能不是必需的，暂时保持原样)
# ... (但需要修改 build_model_from_openai_state_dict 以适应新的模型结构，但这主要用于加载预训练的 OpenAI CLIP 模型，对于您的新任务可能不适用)
# ... (为保持简洁，我们暂时跳过这些函数的修改，因为它们主要服务于视觉模型)

# --- [重要提示] ---
# 为了让 `factory.py` 能正确识别 `MultiModalModel`，我们将原来的 `CLIP` 指向新的 `MultiModalModel`。
# 这是一个简便的 hack，避免了在 factory 中进行大量的逻辑修改。
CLIP = MultiModalModel

# ... (文件剩余部分可以暂时保持不变，因为它们大多是针对视觉模型的辅助函数)
# ... (例如 resize_pos_embed, trace_model 等)
...
def convert_weights_to_lp(model: nn.Module, dtype=torch.float16):
    ...
    def _convert_weights(l):
        ...
        if isinstance(l, TimeSeriesTransformer):
            # convert timeseries nn.Parameter projections
            attr = getattr(l, "proj", None)
            if attr is not None:
                attr.data = attr.data.to(dtype)
