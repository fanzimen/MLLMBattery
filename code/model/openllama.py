...
class OpenLLAMAPEFTModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if 'masks' in inputs:
            ...
            loss_pixel = 0
            feats_text_tensor = (self.visual_encoder, class_name, self.device)
            ...
            loss = outputs.loss #HuggingFace 的 LlamaForCausalLM 会内部自动计算 Causal Language Modeling (CLM) Loss（即下一个 Token 预测的交叉熵损失）
            # 这里的 loss 仅仅是文本生成任务的损失（即模型回答得对不对）
            # calculate the token accuarcy 下面代码是为了监控训练过程中的模型性能，关心模型生成的 Assistant 回复 是否与 output_texts 一致
            chosen_tokens = torch.max(outputs.logits, dim=-1)[1][:, 1:-1]    # [B, S-1]
            labels = targets[:, 2:]
            gen_acc = (chosen_tokens.reshape(-1) == labels.reshape(-1)).to(torch.long)    # [B*S]
            valid_mask = (labels != -100).reshape(-1)
